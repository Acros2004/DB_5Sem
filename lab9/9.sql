...
select *from AUDITORIUM_TYPE;
select *from AUDITORIUM;
declare --уже изменил на 110-5
    n BOOLEAN :=  false;
    auditorium_rec auditorium%rowtype;
begin
    update auditorium set auditorium = '110-4',
                          auditorium_name = '110-4',
                          auditorium_capacity = 90,
                          auditorium_type = 'ЛК'
                        where auditorium = '110-5';
    if n then   commit;
        else rollback;
    end if;
exception
...
select *from auditorium where auditorium like '110%';
...
declare 
    auditorium_rec auditorium%rowtype;
begin
    insert into auditorium VALUES ('206-4','206-4', 60, 'LH');
    commit;
exception
      when others
        then dbms_output.put_line(sqlcode||' '||sqlerrm);
...
select *from auditorium where auditorium like '206%';
insert into auditorium values ('206-5','206-5',60,'ЛК');
declare 
    auditorium_rec auditorium%rowtype;
begin
...
declare
  ...
  aum auditorium.auditorium%type;
  rec auditorium%rowtype;
  cap auditorium.auditorium_capacity%type;
begin
  for xxx in cur(30)
      loop
      dbms_output.put_line(xxx.auditorium||' '||xxx.auditorium_capacity);
     if xxx.auditorium_capacity >= 90
          then delete auditorium where rowid = xxx.rowid and xxx.auditorium_capacity >= 90;
    elsif xxx.auditorium_capacity >= 60
